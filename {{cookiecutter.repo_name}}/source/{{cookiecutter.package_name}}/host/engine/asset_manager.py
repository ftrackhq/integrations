...
class {{cookiecutter.host_type_capitalized}}AssetManagerEngine(AssetManagerEngine):
    ...
    def discover_assets(self, assets=None, options=None, plugin=None):
        ...
        ftrack_asset_nodes = {{cookiecutter.host_type}}_utils.get_ftrack_nodes()
    ...
    def change_version(self, asset_info, options, plugin=None):
        ...
        *plugin* : Default None. Plugin definition, a dictionary with the
        ...
        # Reload asset
        try:
            {{cookiecutter.host_type}}_utils.load_reference_node(reference_node)
            status = core_constants.SUCCESS_STATUS
    ...
    def load_asset(self, asset_info, options=None, plugin=None):
        ...
        for node in nodes:
            if cmds.nodeType(node) == 'reference':
                reference_node = {{cookiecutter.host_type}}_utils.getReferenceNode(node)
                if reference_node:
                    break
    ...
    def unload_asset(self, asset_info, options=None, plugin=None):
        ...
        if reference_node:
            ...
            try:
                {{cookiecutter.host_type}}_utils.unload_reference_node(reference_node)
                result.append(str(reference_node))
                status = core_constants.SUCCESS_STATUS
            except Exception as error:
                message = str(
            ...
            for node in nodes:
                ...
                try:
                    if {{cookiecutter.host_type}}_utils.obj_exists(node):
                        {{cookiecutter.host_type}}_utils.delete_object(node)
                        result.append(str(node))
                        status = core_constants.SUCCESS_STATUS
                except Exception as error:
                    message = str(
    ...
    def remove_asset(self, asset_info, options=None, plugin=None):
        ...
        if reference_node:
            self.logger.debug("Removing reference: {}".format(reference_node))
            try:
                {{cookiecutter.host_type}}_utils.remove_reference_node(reference_node)
                result.append(str(reference_node))
                status = core_constants.SUCCESS_STATUS
            except Exception as error:
                message = str(
        ...
        if {{cookiecutter.host_type}}_utils.obj_exists(self.dcc_object.name):
            try:
                {{cookiecutter.host_type}}_utils.delete_object(self.dcc_object.name)

...
class AmDefaultResolver(BasePlugin):
    name = "am_default_resolver"
    ...
    def resolve_task_dependencies(self, context, options):
        try:
            ...
            if resolver_name not in TASK_TYPE_RESOLVERS:
                resolver_name = "*"
    ...
    def run(self, store):
        ...
        # session.get instead of `Context where id={}`.format(): same
        # family as the Aikido-B608 fixes elsewhere in this PR (B608
        # itself doesn't fire here because the format string lacks the
        # `from` keyword, but the structural fix removes the
        # interpolation entirely). Auto-populate behaviour for the
        # downstream `resolve_task_dependencies` walk is preserved.
        context = self.session.get("Context", context_id)
        if context is None:
            raise Exception(
                "The context {} is not known to ftrack!".format(context_id)
            )
        elif context.entity_type != "Task":
            raise Exception(
                "Asset resolve can only be performed on tasks, not {}!".format(
                    context_id
                )
            )
        ...
        result = self.resolve_task_dependencies(context, self.options)
        store["versions"] = result.get("versions", [])
        return result

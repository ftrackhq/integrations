...
class AssetListModel(QtCore.QAbstractTableModel):
    ...
    def event_manager(self):
        """Return :class:`~ftrack_framework_core.event.EventManager` instance"""
    ...
    def rowCount(self, index=QtCore.QModelIndex):
        ...
        return len(self._asset_entities_list)
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if role == QtCore.Qt.DisplayRole:
            if index.row() < len(self._asset_entities_list):
                return self._asset_entities_list[index.row()]
        return None
    ...
    def insertRows(self, row, data, index=None):
        ...
        for n in range(count):
            if row + n < len(self._asset_entities_list):
                self._asset_entities_list.insert(row + n, data[n])
            else:
                self._asset_entities_list.append(data[n])
        self.endInsertRows()

    def getIndex(self, asset_info_id):
        """Return index of asset having id provided in *asset_info_id*"""
        row = -1
        for _row, asset_info in enumerate(self._asset_entities_list):
    ...
    def getDataById(self, asset_info_id):
        ...
        for index, asset_info in enumerate(self._asset_entities_list):
            if asset_info[asset_const.ASSET_INFO_ID] == asset_info_id:
                return asset_info
        self.logger.warning(
            'No asset info found for id {}'.format(asset_info_id)
    ...
    def setData(self, index, asset_info, silent=False, roles=None):
        ...
        self._asset_entities_list[index.row()] = asset_info
        if not silent:

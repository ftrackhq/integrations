...
class WiretapApi(object):
    ...
    def create_project(self, project_name):
        ...
        xml_settings = self._xml_settings(ftrack_project=project_name)

        if not self._child_node_exists("/projects", project_name, WiretapNodeType.Project):
            logger.info(f"Project '{project_name}' does not exists. Create it with settings: \n{pprint.pformat(settings)}")
    ...
    def _xml_settings(self, ftrack_project, partition, width=1920, height=1080,depth='10-bit'):
        ...
        fps = str(ftrack_project['custom_attributes']['fps'])
        project_xml_data = f"""
        <Project>
        <Name>{ftrack_project['name']}</Name>
        <Description>"Created with ftrack integration for flame: {self.flame_version}"</Description>

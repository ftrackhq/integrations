...
class FlameSceneSavedValidatorPlugin(BasePlugin):
    '''
    Plugin to validate if the scene has been saved.
    '''

    name = 'flame_scene_saved_validator'
    ...
    def run(self, store):
        ...
        if not scene_name:
            ...
            self.logger.warning('Flame scene has never been saved.')
            raise PluginValidationError(
                message='Flame scene has never been saved, Click fix to save it to a temp file',
                on_fix_callback=self.save_to_temp,
                fix_kwargs={'extension_format': extension_format},
            )
        if not scene_saved:
            self.logger.warning('Flame scene not saved')
            raise PluginValidationError(
                message='Flame scene not saved, Click fix to save it.',

...
class AfterEffectsProjectExporterPlugin(BasePlugin):
    name = "aftereffects_project_exporter"
    ...
    def run(self, store):
        ...
        project_path = store["components"][component_name]["project_path"]

        new_file_path = get_temp_path(filename_extension="aep")

        self.logger.debug(
            f"Copying After Effects project from {project_path} to {new_file_path}"
        )

...
aftereffects_rpc_connection = None
startup_tools = []
session = None
...
launcher_expected_aftereffects_pid = None
...
def _get_application_icon():
    """Return icon for After Effects standalone helper app."""
    ...
    for candidate in package_candidates:
        if os.path.isfile(candidate):
            ...
            if icon.isNull():
                ...
                continue
            logger.info("Using packaged helper icon: %s", candidate)
            return icon
...
def on_connected_callback(event):
    """After Effects has connected, run bootstrap tools."""
...
def process_watchdog_callback():
    """Check After Effects process + panel responsiveness periodically."""
    ...
    if integration_alive_seconds % 40 == 0:
        logger.info(
            ...
            f"connected: {aftereffects_rpc_connection.connected}"
        )

    # Check if After Effects still is running
    if not process_monitor.check_running():
        logger.warning("After Effects process gone, shutting down!")
    ...
    # Check if After Effects panel is alive
    check_start = time.perf_counter()
    respond_result = aftereffects_rpc_connection.check_responding(
        timeout_ms=PANEL_HEALTHCHECK_TIMEOUT_MS,
    )
    check_elapsed_ms = int((time.perf_counter() - check_start) * 1000)
    logger.debug(
        "After Effects panel healthcheck result=%s elapsed=%sms timeout=%sms",
        respond_result,
        check_elapsed_ms,
        PANEL_HEALTHCHECK_TIMEOUT_MS,
    )
    if not respond_result and aftereffects_rpc_connection.connected:
        aftereffects_rpc_connection.connected = False
        logger.info(
            "After Effects is not responding but process "
            f"({process_monitor.process_pid}) is still there, "
            "panel temporarily closed?"
        )
    elif respond_result and not aftereffects_rpc_connection.connected:
        aftereffects_rpc_connection.connected = True
        logger.info("After Effects is responding again, panel alive.")
...
def _is_matching_aftereffects_pid(pid, aftereffects_version):
    """Return True if *pid* appears to be the expected After Effects process."""
    try:
        if sys.platform == "darwin":
            ...
            expected_command_fragments = [
                f"Adobe After Effects {str(aftereffects_version)}.app",
                "MacOS/After Effects",
                "Adobe After Effects",
            ]
            return any(
...
def probe_aftereffects_pid(
    aftereffects_version,
    expected_pid=None,
    expected_application_path=None,
):
    """
    Probe the After Effects PID based on the version for the process monitor.

    :param aftereffects_version: The version of After Effects to probe.
    ...
    if expected_pid:
        if _is_matching_aftereffects_pid(expected_pid, aftereffects_version):
            logger.debug("Probe matched launcher PID hint: %s", expected_pid)
            return expected_pid

        if not launcher_expected_pid_fallback_logged:
            logger.warning(
                "Launcher provided PID hint (%s) did not match After Effects "
                "process. Falling back to executable probe.",
                expected_pid,
            )
    ...
    if sys.platform == "darwin" and expected_application_path:
        for pid, command in _iter_mac_processes():
            if _is_matching_macos_application_path(
                command, expected_application_path
            ):
                logger.info(
                    "Matched After Effects process by application path hint: "
                    "pid=%s path=%s",
                    pid,
    ...
    if sys.platform == "darwin":
        executable_label = f"Adobe After Effects {str(aftereffects_version)}"
        logger.info(f"Probing Mac PID (executable: {executable_label})")

        expected_command_fragments = [
            f"Adobe After Effects {str(aftereffects_version)}.app/Contents/MacOS/After Effects",
            "MacOS/After Effects",
            "Adobe After Effects",
        ]
        for pid, command in _iter_mac_processes():
    ...
    logger.warning("After Effects not found running!")
    return None


def bootstrap_integration(framework_extensions_path):
    """Initialise After Effects standalone integration."""

    global \
        client_instance, \
        aftereffects_rpc_connection, \
        startup_tools, \
        session, \
        process_monitor, \
        process_watchdog_timer, \
        last_panel_healthcheck_timestamp, \
        launcher_expected_aftereffects_pid, \
        launcher_expected_application_path, \
        launcher_expected_pid_fallback_logged, \
        launcher_expected_path_fallback_logged

    logger.debug(
        "After Effects standalone integration initialising, extensions path:"
    ...
    aftereffects_rpc_connection = JavascriptRPC(
        "aftereffects",
        session,
        client_instance,
        panel_launchers,
    ...
    set_usage_tracker(
        UsageTracker(
            ...
            default_data=dict(
                ...
                version=__version__,
                app_version=aftereffects_rpc_connection.dcc_version,
                os=platform.platform(),
            ),
        )
    )

    launcher_expected_aftereffects_pid = _extract_launcher_expected_pid()
    launcher_expected_application_path = (
    ...
    if launcher_expected_aftereffects_pid:
        logger.info(
            "Using launcher provided After Effects PID hint: %s",
            launcher_expected_aftereffects_pid,
        )

    if launcher_expected_application_path:
        logger.info(
            "Using launcher provided After Effects application path hint: %s",
            launcher_expected_application_path,
        )

    logger.info(
        "After Effects watchdog probe config: version=%s, expected_pid=%s, "
        "expected_application_path=%s, session_id=%s",
        aftereffects_rpc_connection.dcc_version,
        launcher_expected_aftereffects_pid,
        launcher_expected_application_path,
        aftereffects_rpc_connection.remote_integration_session_id,
    )

    process_monitor = MonitorProcess(
        partial(
            probe_aftereffects_pid,
            aftereffects_rpc_connection.dcc_version,
            launcher_expected_aftereffects_pid,
            launcher_expected_application_path,
        )
    ...
    for _ in range(30 * 2):  # Wait 30s for After Effects to connect
        time.sleep(0.5)

        if process_monitor.check_running():
            break

        logger.debug("Still waiting for After Effects to launch")

    else:
        raise RuntimeError(
            "After Effects "
            f"{aftereffects_rpc_connection.remote_integration_session_id} "
            f"({aftereffects_rpc_connection.remote_integration_session_id}) "
            "process never started. Shutting down."
        )

    logger.warning(
        f"After Effects {aftereffects_rpc_connection.dcc_version} standalone "
    ...
    logger.info(
        "After Effects process watchdog started with %ss interval.",
        PROCESS_WATCHDOG_INTERVAL_SECONDS,
    )


def run_integration():
    """Run After Effects standalone integration as long as AE is alive."""
...
try:
    ...
    # Make sure any exception that happens are logged as there is no console.

...
def _get_uxp_bootstrap_folder():
    """Return UXP bootstrap folder for After Effects."""
    return os.path.join(_get_connect_user_data_dir(), NAME, "uxp-bootstrap")


def _get_uxp_bootstrap_path():
    """Return latest UXP bootstrap path for After Effects."""
    return os.path.join(
...
def _write_uxp_bootstrap_data(
    ...
    bootstrap_data = {
        ...
        "aftereffects_version": str(aftereffects_version),
    ...
    logger.info(
        "Wrote After Effects UXP bootstrap file for session %s to %s",
        remote_integration_session_id,
        bootstrap_path,
    )
...
def on_launch_integration(session, event):
    ...
    if hasattr(application_version, "version"):
        aftereffects_version = application_version.version[0]
    else:
        aftereffects_version = int(str(application_version).split(".")[0])

    logger.info(
        "Preparing After Effects launch using the UXP integration bootstrap flow"
    )
    ...
    launch_data["integration"]["env"][
        "FTRACK_AFTEREFFECTS_UXP_BOOTSTRAP_PATH"
    ] = bootstrap_path
    launch_data["integration"]["env"][
        "FTRACK_AFTEREFFECTS_UXP_BOOTSTRAP_DIR"
    ] = os.path.dirname(bootstrap_path)
    launch_data["integration"]["env"][
        "FTRACK_AFTEREFFECTS_UXP_BOOTSTRAP_LATEST_PATH"
    ] = bootstrap_path

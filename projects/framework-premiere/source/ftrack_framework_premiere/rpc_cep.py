...
class PremiereRPCCEP(object):
    '''Base Premiere remote connection for CEP based integration.'''
    ...
    @property
    def premiere_version(self):
        '''Return Premiere version.'''
        return self._premiere_version

    @premiere_version.setter
    def premiere_version(self, value):
        '''Set Premiere version to *value*.'''
        self._premiere_version = value
    ...
    @property
    def connected(self):
        '''Return True if connected to Premiere.'''
    ...
    def connected(self, value):
        ...
        if self._connected:
            self.logger.info(
                f'Successfully established connection to Premiere {self.premiere_version}'
            )

    def __init__(
        self, session, client, panel_launchers, on_run_dialog_callback
    ):
        super(PremiereRPCCEP, self).__init__()

        # Store reference to self in class variable
        PremiereRPCCEP._instance = self
    ...
    def instance():
        ...
        assert PremiereRPCCEP._instance, 'Premiere RPC not created!'
        assert (
            PremiereRPCCEP._instance.connected
        ), 'Premiere not connected, please keep panel open while integration is working!'

        return PremiereRPCCEP._instance

    def _initialise(self):
        '''Initialise the Premiere connection'''

        self.premiere_version = os.environ.get('FTRACK_PREMIERE_VERSION')
        assert (
            self.premiere_version
        ), 'Premiere integration requires FTRACK_PREMIERE_VERSION passed as environment variable!'
        ...
        assert (
            self.remote_integration_session_id
        ), 'Premiere integration requires FTRACK_REMOTE_INTEGRATION_SESSION_ID passed as environment variable!'
        ...
        self.logger.info(
            f'Created Premiere {self.premiere_version} connection (session id: {self.remote_integration_session_id})'
        )
    ...
    def check_responding(self):
        '''Check if Premiere is alive, send context data'''
    ...
    def rpc(self, function_name, args=None, callback=None, fetch_reply=True):
        ...
        *callback* providing the reply (async) or
        ...
        self.logger.debug(f'Running Premiere RPC call: {data}')
        ...
        self.logger.debug(f'Got Premiere RPC response: {result}')

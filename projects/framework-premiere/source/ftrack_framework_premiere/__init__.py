...
from .rpc_cep import PremiereRPCCEP
...
def bootstrap_integration(framework_extensions_path):
    ...
    global premiere_connection, remote_session, process_monitor
    ...
    premiere_connection = PremiereRPCCEP(
        remote_session,
        client_instance,
        dcc_config['tools'],
    ...
    set_usage_tracker(
        UsageTracker(
            ...
            default_data=dict(
                ...
                version=__version__,
                app_version=premiere_connection.premiere_version,
    ...
    process_monitor = process_util.MonitorProcess(
        premiere_connection.premiere_version
    )
    ...
    for _ in range(30 * 2):  # Wait 30s for Premiere to connect
        ...
        raise RuntimeError(
            ...
            f'({premiere_connection.remote_integration_session_id}) '
            'process never started. Shutting down.'
        )

    logger.warning(
        f'Premiere {premiere_connection.premiere_version} standalone '
    ...
    while True:
        ...
        if active_time % 10000 == 0:
            logger.info(
                ...
                f'connected: {premiere_connection.connected}'
            )
        ...
        if active_time % (5 * 1000) == 0:
            ...
            if not process_monitor.check_running():
                ...
                respond_result = premiere_connection.check_responding()
                if not respond_result and premiere_connection.connected:
                    premiere_connection.connected = False
                    logger.info(
                        f'Premiere is not responding but process ({process_monitor.process_pid}) '
                        f'is still there, panel temporarily closed?'
                    )
                elif respond_result and not premiere_connection.connected:
                    premiere_connection.connected = True

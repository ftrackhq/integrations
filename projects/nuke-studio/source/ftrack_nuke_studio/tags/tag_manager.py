...
class TagManager(FtrackBase):
    ...
    def _setTasksTags(self):
        ...
        for task_type in self.session.query("Type"):
            ...
            meta = ftag.metadata()
            meta.setValue("tag.type", "ftrack")
            meta.setValue("tag.value", task_type_name)
            meta.setValue("tag.ftrack_type", "task")
            meta.setValue("tag.ftrack_id", task_type_id)
            meta.setValue("tag.ftrack_name", task_type_name)
        ...
        self.logger.debug("Added task type tags: {0}".format(task_type_tags))

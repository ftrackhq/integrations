...
class FtrackBasePreset(FtrackBase):
    def __init__(self, name=None, properties=None, **kwargs):
        properties = properties or {}
        '''Initialise class with *name* and *properties*,'''
        super(FtrackBasePreset, self).__init__(name=name, properties=properties)
    ...
    def resolve_ftrack_project_structure(self, task):
        ...
        if not isinstance(track_item, hiero.core.Sequence):
            ...
            for result in results:
                ...
                composed_result = '{}:{}'.format(
                    result['object_type'], sanitised_result
                )
    ...
    def resolve_ftrack_version(self, task):
        ...
        has_data = (
            self._components.get(task._item.parent().name(), {})
            .get(task._item.name(), {})

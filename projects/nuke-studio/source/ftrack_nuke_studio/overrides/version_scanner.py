...
def register_versioning_overrides():
    ...
    if hiero_version_tuple < (11, 3, 0):
        ...
        if not hasattr(VersionScanner, "_default_createAndInsertClipVersion"):
            ...
            VersionScanner.createAndInsertClipVersion = (
                ftrack_create_and_insert_clip_version
            )
...
def ftrack_find_version_files(scanner_instance, version):
    ...
    filtered_paths = filter(
        lambda v: scanner_instance.filterVersion(binitem, v),
        hiero_ordered_versions,
    )
...
def ftrack_filter_version(scanner_instance, bin_item, new_version_file):
    ...
    )  # let's check if the first version has it...
    if not bin_ftrack_tag:
        return scanner_instance._default_filterVersion(
            bin_item, new_version_file
        )
...
def ftrack_create_clip(scanner_instance, new_filename):
    ...
    if new_filename in scanner_instance._ftrack_component_reference:
        ...
        file_path = current_ftrack_location.get_filesystem_path(
            new_filename
        ).split()[0]
...
def ftrack_insert_clips(scanner_instance, bin_item, clips):
    ...
    new_versions.extend(
        scanner_instance._default_insertClips(bin_item, non_entities)
    )
    for component in entities:
        version = add_clip_as_version(
            component, bin_item, scanner_instance._ftrack_component_reference
        )
...
# NS >= 11.3vX
def ftrack_create_and_insert_clip_version(
    scanner_instance, bin_item, new_filename
):
    if new_filename not in scanner_instance._ftrack_component_reference:
        return scanner_instance._default_createAndInsertClipVersion(
            bin_item, new_filename
        )

    clip = ftrack_create_clip(scanner_instance, new_filename)
    version = add_clip_as_version(
        clip, bin_item, scanner_instance._ftrack_component_reference
    )

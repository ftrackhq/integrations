...
logger = logging.getLogger('framework_3dsmax.discover')
...
def on_discover_framework_3dsmax(session, event):

    from framework_3dsmax import (
...
def on_launch_framework_3dsmax(session, event):
    pipeline_max_base_data = on_discover_framework_3dsmax(session, event)
...
def register(session):
    ...
    handle_discovery_event = functools.partial(
        on_discover_framework_3dsmax, session
    )
    ...
    handle_launch_event = functools.partial(on_launch_framework_3dsmax, session)
    session.event_hub.subscribe(

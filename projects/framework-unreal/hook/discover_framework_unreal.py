...
logger = logging.getLogger('framework_unreal.discover')
...
def on_discover_framework_unreal(session, event):
    from framework_unreal import (
...
def on_launch_framework_unreal(session, event):
    '''Handle application launch and add environment to *event*.'''

    pipeline_unreal_base_data = on_discover_framework_unreal(session, event)
...
def register(session):
    ...
    handle_discovery_event = functools.partial(
        on_discover_framework_unreal, session
    )
    ...
    handle_launch_event = functools.partial(on_launch_framework_unreal, session)

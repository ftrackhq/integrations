...
def on_launch_pipeline_photoshop(session, event):
    ...
    if not use_uxp and sys.platform == 'darwin':
        ...
        if subprocess.check_output("arch").decode('utf-8').find('i386') == 0:
            logger.warning(
                'Running on Apple Silicon, will require PS to be launched in Rosetta mode!'
            )
            pipeline_photoshop_base_data['integration']['env'][
                'FTRACK_LAUNCH_ARCH'
            ] = 'x86_64'
...
def update_uxp_plugin():
    ...
    if platform.system() == 'Darwin':
        path_uxp_tool = "/Library/Application Support/Adobe/Adobe Desktop Common/RemoteComponents/UPI/UnifiedPluginInstallerAgent/UnifiedPluginInstallerAgent.app/Contents/MacOS/UnifiedPluginInstallerAgent"

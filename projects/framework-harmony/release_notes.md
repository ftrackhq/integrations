...
* [fix] Suppress the stray console windows that briefly flashed on Windows while the standalone process polled Harmony's PID via `tasklist`; the `subprocess.check_output` calls now pass `creationflags=CREATE_NO_WINDOW`.

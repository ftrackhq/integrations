...
class EventHubThread(threading.Thread):
    ...
    def __init__(self, session):
        ...
        self.logger.debug(f"Initializing new EventHubThread for session {session}")
        self.logger.debug(f"Name set for the thread: {_name}")
        self._session = session

    def start(self):
        """Start thread for *_session*."""
        self.logger.debug(f"Starting EventHubThread for session {self._session}")
        super(EventHubThread, self).start()

    def run(self):
        """Listen for events."""
        self.logger.debug(f"EventHubThread started for session {self._session}")

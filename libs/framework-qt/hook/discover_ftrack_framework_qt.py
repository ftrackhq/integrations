...
def on_discover_ftrack_framework_qt(session, event):
    from ftrack_framework_qt import __version__ as integration_version
...
def on_launch_ftrack_framework_qt(session, event):
    '''Handle application launch and add environment to *event*.'''
    logger.debug('launching: {}'.format(NAME))
    qt_base_data = on_discover_ftrack_framework_qt(session, event)
...
def register(session):
    ...
    handle_discovery_event = functools.partial(
        on_discover_ftrack_framework_qt, session
    )

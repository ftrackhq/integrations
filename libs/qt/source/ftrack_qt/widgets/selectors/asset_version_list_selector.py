...
from ftrack_qt.widgets import thumbnails
from ftrack_qt.widgets.overlay.busy_indicator import (
    BusyIndicator,
)
...
class AssetVersionListItem(QtWidgets.QFrame):
    ...
    def build(self):
        self.thumbnail_widget = thumbnails.AssetVersionThumbnail(self.session)
        ...
        self.version_combobox = ftrack_qt.widgets.selectors.VersionSelector(
...
class AssetVersionListSelector(QtWidgets.QFrame):
    '''Widget for selecting an asset version, presented as a list.'''

    versionChanged = QtCore.Signal(object)
    ''' Emitted when the selected asset has changed, with the following arguments: 
    *assetversion_entity*'''

    def __init__(self, session, filters=None, parent=None):
        super(AssetVersionListSelector, self).__init__(parent=parent)

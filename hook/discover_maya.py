...
def on_launch_pipeline_maya(session, event):
    ...
    if selection:
        ...
        pipeline_maya_base_data['integration']['env']['FTRACK_CONTEXTID.set'] =  task['id']
        pipeline_maya_base_data['integration']['env']['FS.set'] = task['parent']['custom_attributes'].get('fstart', '1.0')
        pipeline_maya_base_data['integration']['env']['FE.set'] = task['parent']['custom_attributes'].get('fend', '100.0')
        pipeline_maya_base_data['integration']['env']['FPS.set'] = task['parent']['custom_attributes'].get('fps', '24.0')

...
class HarmonyAssetManagerEngine(AssetManagerEngine):
    ...
    def __init__(
        ...
        and *asset_type_name*'''
        super(HarmonyAssetManagerEngine, self).__init__(
    ...
    def change_version(self, asset_info, options, plugin=None):
        ...
        *plugin* : Default None. Plugin definition, a dictionary with the
        ...
        if (
            ...
            return super(HarmonyAssetManagerEngine, self).change_version(
        ...
        # Reload asset
        try:
            harmony_utils.load_reference_node(reference_node)
            status = core_constants.SUCCESS_STATUS
    ...
    def select_assets(self, assets, options=None, plugin=None):
        ...
        *assets*: List of :class:`~ftrack_connect_pipeline.asset.FtrackAssetInfo`
        '''
        return super(HarmonyAssetManagerEngine, self).select_assets(
    ...
    def load_asset(self, asset_info, options=None, plugin=None):
        ...
        if (
            ...
            return super(HarmonyAssetManagerEngine, self).load_asset(
                asset_info=asset_info, options=options, plugin=plugin
            )
    ...
    def unload_asset(self, asset_info, options=None, plugin=None):
        ...
        if reference_node:
            ...
            try:
                harmony_utils.unload_reference_node(reference_node)
                result.append(str(reference_node))
                status = core_constants.SUCCESS_STATUS
            except Exception as error:
                message = str(
            ...
            for node in nodes:
                ...
                try:
                    if harmony_utils.obj_exists(node):
                        harmony_utils.delete_object(node)
                        result.append(str(node))
                        status = core_constants.SUCCESS_STATUS
                except Exception as error:
                    message = str(
    ...
    def remove_asset(self, asset_info, options=None, plugin=None):
        ...
        if reference_node:
            self.logger.debug("Removing reference: {}".format(reference_node))
            try:
                harmony_utils.remove_reference_node(reference_node)
                result.append(str(reference_node))
                status = core_constants.SUCCESS_STATUS
            except Exception as error:
                message = str(
        ...
        if harmony_utils.obj_exists(self.dcc_object.name):
            try:
                harmony_utils.delete_object(self.dcc_object.name)

...
class LaunchAction(object):
    ...
    def _get_version_from_lists(self, entity_id):
        asset_version_lists = self.session.query('AssetVersionList where id is {0}'.format(entity_id)).one()
        return ','.join([version['id'] for version in asset_version_lists['items']])
...
class ApplicationStore(ftrack_connect.application.ApplicationStore):
    ...
    def _discoverApplications(self):
        ...
        if sys.platform == 'darwin':
            ...
            applications.extend(self._searchFilesystem(
                expression=prefix + ['cineSync', 'cineSync'],
                label='cineSync',
                applicationIdentifier='cineSync',
                icon='cineSync'
            ))

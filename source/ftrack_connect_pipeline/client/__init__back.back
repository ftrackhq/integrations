...
class BasePipelineClient(object):
    ...
    def __init__(self, ui, host, hostid=None):
        ...
        if not self.hostid:
            ...
            self._context = None#self.session.get('Context', context_id)
    ...
    def _host_discovered(self, event):
        ...
        print '_host_discovered : {}'.format(event['data'])
        hostid = str(event['data']['hostid'])
        context_id = str(event['data']['context_id'])
        self._hostsList.add({"host_id":hostid, "context_id":context_id})

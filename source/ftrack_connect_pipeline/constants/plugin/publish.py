# :coding: utf-8
# :copyright: Copyright (c) 2019 ftrack

# from ftrack_connect_pipeline.constants.plugin import _PLUGIN_FINALISER_TYPE
# from ftrack_connect_pipeline.constants.plugin import _PLUGIN_CONTEXT_TYPE
# from ftrack_connect_pipeline.constants.plugin import _PLUGIN_COLLECTOR_TYPE
# from ftrack_connect_pipeline.constants.plugin import _PLUGIN_VALIDATOR_TYPE
# from ftrack_connect_pipeline.constants.plugin import _PLUGIN_OUTPUT_TYPE

from ftrack_connect_pipeline.constants import plugin

#Publisher plugins
PLUGIN_PUBLISHER_FINALISER_TYPE = 'publisher.{}'.format(plugin._PLUGIN_FINALISER_TYPE)
PLUGIN_PUBLISHER_CONTEXT_TYPE = 'publisher.{}'.format(plugin._PLUGIN_CONTEXT_TYPE)
PLUGIN_PUBLISHER_COLLECTOR_TYPE = 'publisher.{}'.format(plugin._PLUGIN_COLLECTOR_TYPE)
PLUGIN_PUBLISHER_VALIDATOR_TYPE = 'publisher.{}'.format(plugin._PLUGIN_VALIDATOR_TYPE)
PLUGIN_PUBLISHER_OUTPUT_TYPE = 'publisher.{}'.format(plugin._PLUGIN_OUTPUT_TYPE)
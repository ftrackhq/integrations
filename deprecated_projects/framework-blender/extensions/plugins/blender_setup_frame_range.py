...
class BlenderSetupFrameRangeStartupPlugin(BasePlugin):
    ...
    def run(self, store):
        ...
        self.logger.debug(
            f'calling bpy.app.handlers.load_post.append with {task}'
        )
        st_frame = int(task['parent']['custom_attributes'].get('fstart', 1))
        end_frame = int(task['parent']['custom_attributes'].get('fend', 100))
        fps = int(task['parent']['custom_attributes'].get('fps', 24))
        ...
        def load_handler(*args, **kwargs):
            ...
            if args[0] == '':
                self.logger.debug(
                    f'calling load_handler to set timeline with {st_frame}, {end_frame}, {fps}'
                )
        ...
        self.logger.debug(
            f'registering load_handler to set timeline with {st_frame}, {end_frame}, {fps}'
        )

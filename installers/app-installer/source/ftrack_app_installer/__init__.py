...
class MacOSAppInstaller(AppInstaller):
    ...
    def generate_installer_package(self, codesign=True):
        ...
        # dmgbuild settings
        background_path = os.path.join(self.os_root_folder, "dmg_image.png")

        dmg_settings = {
            "filename": dmg_path,
            "volume_name": self.bundle_name,
            "format": "UDBZ",
            "size": None,
            "files": [self.bundle_path],
            "symlinks": {"Applications": "/Applications"},
            "icon_locations": {
                os.path.basename(self.bundle_path): (130, 180),
        ...
        if codesign:
            ...
            logging.info("Checking if xcrun is available.")
            xcrun_available = os.system("xcrun --version")
            if xcrun_available != 0:
                logging.info(
                    "xcrun can't be found. Trying to install it. Please enter your sudo password when prompted."
                )
